 ...
 * Classes renaming, remove 'st_' prefix.
...
//asio and compiler check
#ifdef _MSC_VER
	static_assert(_MSC_VER >= 1600, "ascs must be compiled with Visual C++ 10.0 or higher.");
	...
	//are 4.9 and 3.1, so, I supply the following compiler verification. If there's something wrong, you can freely modify them,
	//and if you let me know, I'll be very appreciated.
	#ifdef __clang__
		static_assert(__clang_major__ > 3 || __clang_major__ == 3 && __clang_minor__ >= 1, "ascs must be compiled with Clang 3.1 or higher.");
	#elif __GNUC__ < 4 || __GNUC__ == 4 && __GNUC_MINOR__ < 9
		#error ascs must be compiled with GCC 4.9 or higher.
	#else
		...
		#error ascs need c++0x support, please open these features.
	#endif
#else
